...
3.  Create an empty public repository named `RobustSandwichRV`, then
    push:
...
The included workflows run `R CMD check` on Windows, macOS, and Linux
and deploy the pkgdown site to the `gh-pages` branch. Enable GitHub
Pages for that branch after the first successful site build.
